...
docker exec --env VERSION=${VER_INFO} "$(cat "${CIDFILE}")" ./linuxdeploy-x86_64.AppImage --appdir AppDir -e ${TRAVIS_DEBIAN_BUILD_DIR}/usr/bin/qradiolink -i ${BUILD_PATH}/application/res/icon.png -d ${BUILD_PATH}/application/qradiolink.desktop --plugin qt --output appimage
