    ...
    QObject(parent)
{
    ...
    _resampler_time_domain = gr::filter::rational_resampler_ccf::make(1, 10,
                gr::filter::firdes::low_pass(1, INTERNAL_DEFAULT_SAMPLE_RATE, 50000, 25000, gr::fft::window::WIN_HAMMING));
...
void gr_demod_base::set_time_sink_samp_rate(int samp_rate)
{
    ...
    std::vector<float> taps = gr::filter::firdes::low_pass(1, INTERNAL_DEFAULT_SAMPLE_RATE,
                                                           samp_rate/2 - samp_rate/8, samp_rate/4,
                                                            gr::fft::window::WIN_HAMMING);

    _resampler_time_domain = gr::filter::rational_resampler_ccf::make(1, decimation, taps);
    if(_time_domain_enabled)
    {
        try
        {
