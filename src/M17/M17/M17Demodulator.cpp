...
bool M17Demodulator::update(int16_t *samples, int size)
{
    ...
    if(baseband.data != nullptr)
        delete[] baseband.data;
    baseband.data = new int16_t[size];
    memcpy(baseband.data, samples, size * sizeof(int16_t));
    ...
    if(baseband.data != NULL)
    {
        ...
        while(syncword.index != -1)
        {
            ...
            if (syncDetected == false)
            {
                ...
                {
                    //std::cout << "Valid syncword found" << std::endl;
            ...
            else
            {
                ...
                if (frame_index == M17_SYNCWORD_SYMBOLS)
                {
                    ...
                    {
                        // Lock lost
                        //std::cout << "M17 lock lost" << std::endl;
                        syncDetected = false;
                        locked       = false;
                        phase        += 1;
