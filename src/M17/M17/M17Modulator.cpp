                        ...
                        const bool isLast)
{
    ...
    generateBaseband();
    emitBaseband(samples);
}

void M17Modulator::generateBaseband()
{
    memset(idleBuffer, 0x00, M17_FRAME_SAMPLES * sizeof(int16_t));
...
#else
void M17Modulator::emitBaseband(std::vector<float> *samples)
{
    for(size_t i = 0; i < M17_FRAME_SAMPLES; i++)
    {
        auto s = idleBuffer[i];
        samples->push_back(float(s) / 32767.0f);
    }
}
#endif
