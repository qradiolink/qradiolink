...
gr_demod_bpsk::gr_demod_bpsk(QObject *parent, int sps, int samp_rate, int carrier_freq, int filter_width, float mod_index) :
    QObject(parent)
{
    ...
    _top_block->connect(_band_pass_filter_1,0,_clock_recovery,0);
...
void gr_demod_bpsk::start()
{
    _top_block->start();
}

void gr_demod_bpsk::stop()
{
    _top_block->stop();
    _top_block->wait();
}

std::vector<unsigned char>* gr_demod_bpsk::getData()
{
